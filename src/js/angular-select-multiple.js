...
const magicApp = angular.module("ngSelectMultiple", ['angular-sortable-view']);

magicApp.directive("birdSelect", function(){
  return {
     restrict: 'AE',
     require: 'ngModel',
     scope: {
     ...
     controller: ['$scope', '$http', MagicSelectController]
   }
});

const template_string = '<div class="magic-select-container">'
                        + '<input type="text" ng-model="selectSearch.$" placeholder="{{ ngPlaceholder }}" class="magic-select-search"/>'
                        + '<ul sv-root sv-part="selectedList" class="magic-selected-list">'
                          + '<li ng-repeat="id in selectedList" sv-element>'
                            + '<input type="hidden" name="{{ ngName }}[]" value="{{ id }}"/>'
                            + '{{ listNameById(id) }}'
                            + '<span class="magic-selected-remove" ng-click="removeFromSelected(id)">x</span>'
                          + '</li>'
                        + '</ul>'
                        + '<ul class="magic-select-result-list">'
                          + '<li ng-repeat="item in list | filter: selectedFilter | filter: selectSearch | orderBy:\'name\'" ng-click="addToSelected(item)">'
                            + '{{ item.name }}'
                          + '</li>'
                        + '</ul>'
                      + '</div>';

function MagicSelectController($scope, $http){
  ...
  $scope.setList = function(){
    $scope.list = $scope.ngList;
  };

  // List from Url
  $scope.getList = function(){
    $http.get($scope.ngListUrl).success(
      function(data){
        $scope.list = data;
      }
    ).error(
      function(data, status){
        console.error('ERROR: Can not retrieve list from url:'+$scope.ngListUrl, status, data);
      }
    );
  };

  // Filter in Result List
  $scope.selectedFilter = function(item){
    for(key in $scope.selectedList)
      if(item.id == $scope.selectedList[key])
        return false;
    return true;
  };

  $scope.listNameById = function(id){
    for(key in $scope.list)
      if(id == $scope.list[key].id)
        return $scope.list[key].name;
    return  'ID:'+id+' not found in list!';
  };
  ...
  if($scope.ngList) $scope.setList();
  else if($scope.ngListUrl) $scope.getList();
}
